...
let error (loc : Loc.t) message = raise (Lexer.Parse_error (loc, message))
...
let load ?lexer path ~mode =
  ...
  Fun.protect
    ...
    (fun () ->
      let lexbuf = Lexbuf.from_channel ic ~fname:path in
      parse ~mode ?lexer lexbuf)
...
let insert_comments csts comments =
  ...
  let tokens =
    reconciliate [] (Cst.tokenize csts)
      ( List.sort comments ~cmp:compare
